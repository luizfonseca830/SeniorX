 ...
 * 
 */
public class CalculadoraMultiplicacaoImpl {
	...
	public CalculadoraMultiplicacaoImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(FieldCalculadora input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				userId.getTenant(), //
				MyServiceConstants.DOMAIN, //
				MyServiceConstants.SERVICE, //
				MyServiceConstants.Commands.CALCULADORA_MULTIPLICACAO, //
	...
	*/
	public FieldResultadoCalculadora calculadoraMultiplicacao(FieldCalculadora input, long timeout) {
		...
		FieldResultadoCalculadora output = DtoJsonConverter.toDTO(resultMessage.getContent(), FieldResultadoCalculadora.class);
		if (output == null) {
	...
	*/
	...
	public CompletableFuture<FieldResultadoCalculadora> calculadoraMultiplicacaoRequest(FieldCalculadora input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, FieldResultadoCalculadora.class);
	}
	...
	* Realiza a operação matematica de multiplicação
	*/
	public CompletableFuture<FieldResultadoCalculadora> calculadoraMultiplicacaoRequest(FieldCalculadora input, long timeout, TimeUnit unit) {
		MyServiceValidator.validate(input);
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, FieldResultadoCalculadora.class, TimeUnit.MILLISECONDS.convert(timeout, unit));
	}

...
import br.com.senior.mydomain.myservice.ExportAtracaoInput;
import br.com.senior.mydomain.myservice.ExportAtracaoOutput;
 ...
 * 
 */
public class ExportAtracaoImpl {
	...
	public ExportAtracaoImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(ExportAtracaoInput input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				userId.getTenant(), //
				MyServiceConstants.DOMAIN, //
				MyServiceConstants.SERVICE, //
				MyServiceConstants.Commands.EXPORT_ATRACAO, //
	...
	*/
	public ExportAtracaoOutput exportAtracao(ExportAtracaoInput input, long timeout) {
		...
		ExportAtracaoOutput output = DtoJsonConverter.toDTO(resultMessage.getContent(), ExportAtracaoOutput.class);
		if (output == null) {
	...
	*/
	...
	public CompletableFuture<ExportAtracaoOutput> exportAtracaoRequest(ExportAtracaoInput input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, ExportAtracaoOutput.class);
	}
	...
	* 
	*/
	public CompletableFuture<ExportAtracaoOutput> exportAtracaoRequest(ExportAtracaoInput input, long timeout, TimeUnit unit) {
		MyServiceValidator.validate(input);
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, ExportAtracaoOutput.class, TimeUnit.MILLISECONDS.convert(timeout, unit));
	}

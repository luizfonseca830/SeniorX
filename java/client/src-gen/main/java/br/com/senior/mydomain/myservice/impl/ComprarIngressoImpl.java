...
import br.com.senior.mydomain.myservice.ComprarIngressoInput;
import br.com.senior.mydomain.myservice.ComprarIngressoOutput;
 ...
 * 
 */
public class ComprarIngressoImpl {
	...
	public ComprarIngressoImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(ComprarIngressoInput input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				userId.getTenant(), //
				MyServiceConstants.DOMAIN, //
				MyServiceConstants.SERVICE, //
				MyServiceConstants.Commands.COMPRAR_INGRESSO, //
	...
	*/
	public ComprarIngressoOutput comprarIngresso(ComprarIngressoInput input, long timeout) {
		...
		ComprarIngressoOutput output = DtoJsonConverter.toDTO(resultMessage.getContent(), ComprarIngressoOutput.class);
		if (output == null) {
	...
	*/
	...
	public CompletableFuture<ComprarIngressoOutput> comprarIngressoRequest(ComprarIngressoInput input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, ComprarIngressoOutput.class);
	}
	...
	* 
	*/
	public CompletableFuture<ComprarIngressoOutput> comprarIngressoRequest(ComprarIngressoInput input, long timeout, TimeUnit unit) {
		MyServiceValidator.validate(input);
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, ComprarIngressoOutput.class, TimeUnit.MILLISECONDS.convert(timeout, unit));
	}

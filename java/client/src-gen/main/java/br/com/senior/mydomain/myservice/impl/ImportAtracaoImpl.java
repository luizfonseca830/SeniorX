...
import br.com.senior.mydomain.myservice.ImportAtracaoInput;
import br.com.senior.mydomain.myservice.ImportAtracaoOutput;
 ...
 * 
 */
public class ImportAtracaoImpl {
	...
	public ImportAtracaoImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(ImportAtracaoInput input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				userId.getTenant(), //
				MyServiceConstants.DOMAIN, //
				MyServiceConstants.SERVICE, //
				MyServiceConstants.Commands.IMPORT_ATRACAO, //
	...
	*/
	public ImportAtracaoOutput importAtracao(ImportAtracaoInput input, long timeout) {
		...
		ImportAtracaoOutput output = DtoJsonConverter.toDTO(resultMessage.getContent(), ImportAtracaoOutput.class);
		if (output == null) {
	...
	*/
	...
	public CompletableFuture<ImportAtracaoOutput> importAtracaoRequest(ImportAtracaoInput input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, ImportAtracaoOutput.class);
	}
	...
	* 
	*/
	public CompletableFuture<ImportAtracaoOutput> importAtracaoRequest(ImportAtracaoInput input, long timeout, TimeUnit unit) {
		MyServiceValidator.validate(input);
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, ImportAtracaoOutput.class, TimeUnit.MILLISECONDS.convert(timeout, unit));
	}

 ...
 * 
 */
public class RetrieveAtracaoImpl {
	...
	public RetrieveAtracaoImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(Atracao.GetRequest input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				userId.getTenant(), //
				MyServiceConstants.DOMAIN, //
				MyServiceConstants.SERVICE, //
				MyServiceConstants.Commands.RETRIEVE_ATRACAO, //
	...
	*/
	public Atracao retrieveAtracao(Atracao.GetRequest input, long timeout) {
		...
		Atracao output = DtoJsonConverter.toDTO(resultMessage.getContent(), Atracao.class);
		if (output == null) {
	...
	*/
	...
	public CompletableFuture<Atracao> retrieveAtracaoRequest(Atracao.GetRequest input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, Atracao.class);
	}
	...
	* The 'retrieve' request primitive for the Atracao entity.
	*/
	public CompletableFuture<Atracao> retrieveAtracaoRequest(Atracao.GetRequest input, long timeout, TimeUnit unit) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, Atracao.class, TimeUnit.MILLISECONDS.convert(timeout, unit));
	}

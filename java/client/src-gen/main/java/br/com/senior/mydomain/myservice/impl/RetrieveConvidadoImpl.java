 ...
 * 
 */
public class RetrieveConvidadoImpl {
	...
	public RetrieveConvidadoImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(Convidado.GetRequest input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				userId.getTenant(), //
				MyServiceConstants.DOMAIN, //
				MyServiceConstants.SERVICE, //
				MyServiceConstants.Commands.RETRIEVE_CONVIDADO, //
	...
	*/
	public Convidado retrieveConvidado(Convidado.GetRequest input, long timeout) {
		...
		Convidado output = DtoJsonConverter.toDTO(resultMessage.getContent(), Convidado.class);
		if (output == null) {
	...
	*/
	...
	public CompletableFuture<Convidado> retrieveConvidadoRequest(Convidado.GetRequest input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, Convidado.class);
	}
	...
	* The 'retrieve' request primitive for the Convidado entity.
	*/
	public CompletableFuture<Convidado> retrieveConvidadoRequest(Convidado.GetRequest input, long timeout, TimeUnit unit) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, Convidado.class, TimeUnit.MILLISECONDS.convert(timeout, unit));
	}

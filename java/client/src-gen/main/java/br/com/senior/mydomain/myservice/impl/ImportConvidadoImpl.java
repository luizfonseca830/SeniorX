...
import br.com.senior.mydomain.myservice.ImportConvidadoInput;
import br.com.senior.mydomain.myservice.ImportConvidadoOutput;
 ...
 * 
 */
public class ImportConvidadoImpl {
	...
	public ImportConvidadoImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(ImportConvidadoInput input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				userId.getTenant(), //
				MyServiceConstants.DOMAIN, //
				MyServiceConstants.SERVICE, //
				MyServiceConstants.Commands.IMPORT_CONVIDADO, //
	...
	*/
	public ImportConvidadoOutput importConvidado(ImportConvidadoInput input, long timeout) {
		...
		ImportConvidadoOutput output = DtoJsonConverter.toDTO(resultMessage.getContent(), ImportConvidadoOutput.class);
		if (output == null) {
	...
	*/
	...
	public CompletableFuture<ImportConvidadoOutput> importConvidadoRequest(ImportConvidadoInput input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, ImportConvidadoOutput.class);
	}
	...
	* 
	*/
	public CompletableFuture<ImportConvidadoOutput> importConvidadoRequest(ImportConvidadoInput input, long timeout, TimeUnit unit) {
		MyServiceValidator.validate(input);
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, ImportConvidadoOutput.class, TimeUnit.MILLISECONDS.convert(timeout, unit));
	}

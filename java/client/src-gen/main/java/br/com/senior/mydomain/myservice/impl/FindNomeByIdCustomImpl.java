 ...
 * 
 */
public class FindNomeByIdCustomImpl {
	...
	public FindNomeByIdCustomImpl(Supplier<IMessenger> messengerSupplier, UserIdentifier userId, Supplier<Message> messageSupplier) {
		this.messengerSupplier = messengerSupplier;
		this.userId = userId;
		this.messageSupplier = messageSupplier;
	}

	private Message createMessage(FindNomeByIdCustomInput input) {
		if (messageSupplier != null && messageSupplier.get() != null) {
			return messageSupplier.get().followUp( //
				userId.getTenant(), //
				MyServiceConstants.DOMAIN, //
				MyServiceConstants.SERVICE, //
				MyServiceConstants.Commands.FIND_NOME_BY_ID_CUSTOM, //
	...
	*/
	public FindNomeByIdCustomOutput findNomeByIdCustom(FindNomeByIdCustomInput input, long timeout) {
		...
		FindNomeByIdCustomOutput output = DtoJsonConverter.toDTO(resultMessage.getContent(), FindNomeByIdCustomOutput.class);
		if (output == null) {
	...
	*/
	...
	public CompletableFuture<FindNomeByIdCustomOutput> findNomeByIdCustomRequest(FindNomeByIdCustomInput input) {
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, FindNomeByIdCustomOutput.class);
	}
	...
	* 
	*/
	public CompletableFuture<FindNomeByIdCustomOutput> findNomeByIdCustomRequest(FindNomeByIdCustomInput input, long timeout, TimeUnit unit) {
		MyServiceValidator.validate(input);
		...
		Message message = createMessage(input);
		addMessageHeaders(message);
		return serviceRunner.request(message, FindNomeByIdCustomOutput.class, TimeUnit.MILLISECONDS.convert(timeout, unit));
	}

...
public class AtracaoImporterListener implements ImporterListener {
	
	private static final Logger logger = LoggerFactory.getLogger(AtracaoImporterListener.class);
	...
	private void publishEventStatus(ImportEventStatus status) {
		...
		if (runner != null) {
			runner.publish(ServiceContext.get().getCurrentTenant(),  MyServiceConstants.Events.IMPORT_ATRACAO_EVENT, new ImportAtracaoEventPayload(status));
		}
	}
